...
This tab plots residue ID on the x-axis and DAQ score on the y-axis using the same data as the residue table. Click **Refresh** to recompute the scores. If the structure has multiple chains, choose a chain from the **Chain** dropdown; only the selected chain is shown in the plot. Hover over the line to show the nearest residue ID. Drag across a residue range in the plot to select those residues in ChimeraX. Click **Zoom In** to focus the ChimeraX structure view on the last dragged residue range, and click **Zoom Out** to show the full selected structure again. Click **Clear Selection** to clear the current ChimeraX selection.
